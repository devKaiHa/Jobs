...
const getJobUsers = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    const jobsUsers = await _jobSeekersModel.default.find(query).sort({
        createdAt: -1
...
const createJobUser = (0, _expressasynchandler.default)(async (req, res, next)=>{
...
const getJobUser = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    if (!user) return next(new _apiError.default(`No user found for ID: ${id}`, 404));
    res.status(200).json({
        status: "success",
        data: user
    });
});
const updateJobUser = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { id } = req.params;
    const user = await _jobSeekersModel.default.findByIdAndUpdate(id, req.body, {
        new: true
    });
    if (!user) return next(new _apiError.default(`No user found for ID: ${id}`, 404));
    res.status(200).json({
...
const deleteJobUser = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { id } = req.params;
