...
const _jobApplicationModel = /*#__PURE__*/ _interop_require_default(
  require("../../models/jobs/jobApplicationModel")
);
const _jobSeekersModel = /*#__PURE__*/ _interop_require_default(
  require("../../models/jobSeekersModel")
);
const _jobAdvertisementModel = /*#__PURE__*/ _interop_require_default(
  require("../../models/jobs/jobAdvertisementModel")
);
...
const getAllJobApplications = (0, _expressasynchandler.default)(
  async (req, res) => {
    let query = {};
    ...
    const Applications = await _jobApplicationModel.default
      .find(query)
      .skip(skip)
      .limit(limit)
      .sort({
...
const getOneJobApplication = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    const { id } = req.params;
    const Applications = await _jobApplicationModel.default
      .findById(id)
    ...
    if (!Applications) {
      return next(
        new _apiError.default(`No Applications found for this ID: ${id}`, 404)
      );
    }
    res.status(200).json({
      status: "success",
...
const createJobApplication = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    ...
    const Applications = await _jobApplicationModel.default.create(
      applicationData
    );
...
const updateJobApplication = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    const { id } = req.params;
    const Applications = await _jobApplicationModel.default.findByIdAndUpdate(
      id,
      req.body,
      {
        new: true,
...
const deleteJobApplication = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    const { id } = req.params;
    const Applications = await _jobApplicationModel.default.findByIdAndDelete(
      id
    );
    if (!Applications) {
