...
exports.getAllJobApplications = (0, express_async_handler_1.default)(async (req, res) => {
    let query = {};
    ...
    const total = await jobApplicationModel_1.default.countDocuments(query);
    const Applications = await jobApplicationModel_1.default
        .find(query)
        .skip(skip)
        .limit(limit)
        .sort({ createdAt: -1 })
...
exports.getOneJobApplication = (0, express_async_handler_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const Applications = await jobApplicationModel_1.default
        .findById(id)
        .populate("jobSeekerId")
...
exports.createJobApplication = (0, express_async_handler_1.default)(async (req, res, next) => {
    const { jobSeekerId, jobId } = req.body;
    const jobSeeker = await jobSeekersModel_1.default.findById(jobSeekerId);
    if (!jobSeeker || !jobId) {
        return next(new apiError_1.default("Job seeker or jobId not found", 404));
    }
    const applicationData = req.body;
    const Applications = await jobApplicationModel_1.default.create(applicationData);
    await jobAdvertisementModel_1.default.findOneAndUpdate({ _id: jobId }, { $inc: { applicantsNumber: 1 } }, { new: true });
    res.status(201).json({ status: "success", data: Applications });
});
exports.updateJobApplication = (0, express_async_handler_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const Applications = await jobApplicationModel_1.default.findByIdAndUpdate(id, req.body, { new: true });
    if (!Applications)
...
exports.deleteJobApplication = (0, express_async_handler_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const Applications = await jobApplicationModel_1.default.findByIdAndDelete(id);
    if (!Applications) {
