...
const resizeCompanyLogo = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    if (!req.file) return next();
    const filename = `company-logo-${(0, _uuid.v4)()}-${Date.now()}.png`;
    await (0, _sharp.default)(req.file.buffer)
      .toFormat("png")
      .png({
        quality: 70,
      })
...
const getAllJobs = (0, _expressasynchandler.default)(async (req, res) => {
  ...
  const jobs = await _jobAdvertisementModel.default
    .find(query)
    .skip(skip)
    .limit(limit)
    .sort({
...
const getOneJob = (0, _expressasynchandler.default)(async (req, res, next) => {
  ...
  const job = await _jobAdvertisementModel.default
    .findById(id)
    .populate("company");
  if (!job) {
...
const updateJob = (0, _expressasynchandler.default)(async (req, res, next) => {
  ...
  if (req.body.companyInfo && typeof req.body.companyInfo === "string") {
    try {
      ...
      return next(
        new _apiError.default("companyInfo must be a valid object", 400)
      );
    }
  }
  const updateData = {
  ...
  const updatedJob = await _jobAdvertisementModel.default.findByIdAndUpdate(
    id,
    updateData,
    {
      new: true,
