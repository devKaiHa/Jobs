...
exports.resizeCompanyLogo = (0, express_async_handler_1.default)(async (req, res, next) => {
    if (!req.file)
        return next();
...
exports.getAllJobs = (0, express_async_handler_1.default)(async (req, res) => {
    let query = {};
    ...
    const total = await jobAdvertisementModel_1.default.countDocuments(query);
    const jobs = await jobAdvertisementModel_1.default
        .find(query)
        .skip(skip)
        .limit(limit)
        .sort({ createdAt: -1 })
...
exports.getOneJob = (0, express_async_handler_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const userId = req.query.userId;
    const job = await jobAdvertisementModel_1.default.findById(id).populate("company");
    if (!job) {
        return next(new apiError_1.default(`No job found for this ID: ${id}`, 404));
    }
    let isApplied = false;
    if (userId) {
        const existingApplication = await jobApplicationModel_1.default.findOne({
            jobId: id,
            jobSeekerId: userId,
        });
...
exports.createJobs = (0, express_async_handler_1.default)(async (req, res) => {
    const job = await jobAdvertisementModel_1.default.create(req.body);
    const company = await jobsCompaniesModel_1.default.findById(req.body.company);
    if (!company) {
...
exports.updateJob = (0, express_async_handler_1.default)(async (req, res, next) => {
    const { id } = req.params;
    if (!id) {
        return next(new apiError_1.default(`No job found for this ID: ${id}`, 404));
    }
    ...
    const updatedJob = await jobAdvertisementModel_1.default.findByIdAndUpdate(id, updateData, {
        new: true,
    });
    if (!updatedJob) {
...
exports.deleteJob = (0, express_async_handler_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const job = await jobAdvertisementModel_1.default.findByIdAndDelete(id);
    if (!job) {
