...
const processCompanyFiles = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    if (filesField && filesField.logo && filesField.logo[0]) {
        ...
        await (0, _sharp.default)(logoFile.buffer).toFormat("png").png({
            quality: 70
...
const getCompanies = (0, _expressasynchandler.default)(async (req, res)=>{
    ...
    const companies = await _jobsCompaniesModel.default.find(query).sort({
        createdAt: -1
...
const getCompany = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { id } = req.params;
    const company = await _jobsCompaniesModel.default.findById(id).populate("jobAdvertisement");
    if (!company) {
...
const updateCompany = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { id } = req.params;
    const company = await _jobsCompaniesModel.default.findByIdAndUpdate(id, req.body, {
        new: true
    });
    if (!company) {
    ...
    if (req.body.status === "accepted") {
        try {
            ...
            return next(new _apiError.default("Failed to send company data to the main system", 500));
        }
        return;
    }
...
const deleteCompany = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { id } = req.params;
    ...
    await (0, _sendEmail.default)({
        ...
        message: message || `Hello ${company.companyName}, we're sorry to inform you that your registration request has been declined.`
    });
    res.status(200).json({
        status: "success",
