...
const _createToken = /*#__PURE__*/ _interop_require_default(require("../utils/createToken"));
const _jobSeekersModel = /*#__PURE__*/ _interop_require_default(require("../models/jobSeekersModel"));
const _sendEmail = /*#__PURE__*/ _interop_require_default(require("../utils/sendEmail"));
...
const multerFilter = (req, file, cb)=>{
    if (file.mimetype.startsWith("image") || file.mimetype === "application/pdf" || file.mimetype === "application/vnd.openxmlformats-officedocument.wordprocessingml.document" || file.mimetype === "application/msword") {
        cb(null, true);
    } else {
        cb(new _apiError.default("Only images or CV files are allowed", 400), false);
    }
};
...
const processJobSeekerFiles = (0, _expressasynchandler.default)(async (req, res, next)=>{
    if (req.files && req.files.profileImage) {
        const imageFile = req.files.profileImage[0];
        const imageFilename = `profileImage-${(0, _uuid.v4)()}-${Date.now()}.png`;
        await (0, _sharp.default)(imageFile.buffer).toFormat("png").png({
            quality: 70
    ...
    if (req.files && req.files.licenses) {
        ...
        const licenseFilename = `license-${(0, _uuid.v4)()}-${Date.now()}.${licenseExt}`;
        const fs = require("fs");
...
const googleLogin = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    if (!payload) return next(new _apiError.default("Google token not valid", 400));
...
const protect = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    try {
        ...
        if (!currentUser) return next(new _apiError.default("JobSeeker does not exist", 404));
        req.user = currentUser;
        next();
    } catch (error) {
        if (error.name === "TokenExpiredError") return next(new _apiError.default("Token has expired", 401));
        return next(new _apiError.default("Not logged in", 401));
    }
});
const signupJobSeekers = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { name, email, password } = req.body;
    const existingJobSeeker = await _jobSeekersModel.default.findOne({
        email
    });
    if (existingJobSeeker) return next(new _apiError.default("Email already registered", 400));
    const hashedPassword = await _bcrypt.default.hash(password, 10);
    // Generate email verification code
    const verificationCode = Math.floor(100000 + Math.random() * 900000).toString();
    const hashedVerificationCode = await _bcrypt.default.hash(verificationCode, 10);
...
const verifyEmailJobSeekers = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    if (!jobSeeker) return next(new _apiError.default("Verification code invalid or expired", 400));
    const isValid = await _bcrypt.default.compare(verificationCode, jobSeeker.emailVerificationCode);
    if (!isValid) return next(new _apiError.default("Verification code invalid or expired", 400));
...
const loginJobSeekers = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { email, password } = req.body;
    const jobSeeker = await _jobSeekersModel.default.findOne({
        email
    });
    if (!jobSeeker) return next(new _apiError.default("Incorrect email", 401));
    if (!jobSeeker.verified) return next(new _apiError.default("Please verify your email before logging in", 401));
    const passwordMatch = await _bcrypt.default.compare(password, jobSeeker.password);
    if (!passwordMatch) return next(new _apiError.default("Incorrect password", 401));
...
const forgotPasswordJobSeekers = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { email } = req.body;
    const jobSeeker = await _jobSeekersModel.default.findOne({
        email
    });
    if (!jobSeeker) return next(new _apiError.default("No account found with this email", 404));
...
const verifyPasswordResetCodeJobSeekers = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    if (!jobSeeker) return next(new _apiError.default("Reset code invalid or expired", 400));
    const isValid = await _bcrypt.default.compare(resetCode, jobSeeker.passwordResetCode);
    if (!isValid) return next(new _apiError.default("Reset code invalid or expired", 400));
...
const resetPasswordJobSeekers = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { email, newPassword } = req.body;
    const jobSeeker = await _jobSeekersModel.default.findOne({
        email
    });
    if (!jobSeeker) return next(new _apiError.default(`No job seeker found with email ${email}`, 404));
    if (!jobSeeker.resetCodeVerified) return next(new _apiError.default("Reset code not verified", 400));
