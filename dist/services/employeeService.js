...
exports.getEmployees = (0, express_async_handler_1.default)(async (req, res, next) => {
    try {
        ...
        const employees = await employeeModel_1.default.find(query).skip(skip).limit(pageSize);
        res.status(200).json({
            status: "true",
            Pages: totalPages,
            results: totalItems,
...
exports.deleteEmployee = (0, express_async_handler_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const employeeToDelete = await employeeModel_1.default.findByIdAndDelete(id);
    if (!employeeToDelete) {
        return next(new apiError_1.default(`No employee by this id ${id}`, 404));
    }
