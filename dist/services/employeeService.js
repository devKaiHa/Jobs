...
const getEmployees = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    try {
      ...
      const employees = await _employeeModel.default
        .find(query)
        .skip(skip)
        .limit(pageSize);
      res.status(200).json({
...
const getEmployee = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    const { id } = req.params;
...
const updateEmployee = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    const { id } = req.params;
    if (!id) {
    ...
    if (!employee) {
      return next(
        new _apiError.default(`There is no employee with this id ${id}`, 404)
      );
    }
    res.status(200).json({
      status: "true",
...
const deleteEmployee = (0, _expressasynchandler.default)(
  async (req, res, next) => {
    const { id } = req.params;
