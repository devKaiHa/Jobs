...
const _isEmail = /*#__PURE__*/ _interop_require_default(require("../utils/tools/isEmail"));
const _generatePassword = /*#__PURE__*/ _interop_require_default(require("../utils/tools/generatePassword"));
...
const processEmployeeImage = (0, _expressasynchandler.default)(async (req, res, next)=>{
...
const createEmployee = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    if ((0, _isEmail.default)(email)) {
        ...
        return next(new _apiError.default("There is an error in email format", 500));
    }
});
const reSendPassword = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const email = req.body.email;
    //Check if the email format is true or not
    const findEmployee = await _employeeModel.default.findOne({
        email: req.body.email
...
const login = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    if (user.archives === "true") return next(new _apiError.default("Account is not active", 401));
...
const protect = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    try {
        ...
        if (!currentUser) return next(new _apiError.default("Employee does not exist", 404));
...
const forgotPassword = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { email } = req.body;
    const user = await _employeeModel.default.findOne({
        email
    });
    if (!user) return next(new _apiError.default("No account found with this email", 404));
...
const verifyPasswordResetCodePos = (0, _expressasynchandler.default)(async (req, res, next)=>{
    ...
    if (!user) return next(new _apiError.default("Reset code invalid or expired", 400));
    if (!user.passwordResetCode) return next(new _apiError.default("No reset code found", 400));
    const isValid = await _bcrypt.default.compare(resetCode, user.passwordResetCode);
...
const resetPasswordPos = (0, _expressasynchandler.default)(async (req, res, next)=>{
    const { email, newPassword } = req.body;
    const user = await _employeeModel.default.findOne({
        email
    });
    if (!user) return next(new _apiError.default(`No employee found with email ${email}`, 404));
    if (!user.resetCodeVerified) return next(new _apiError.default("Reset code not verified", 400));
